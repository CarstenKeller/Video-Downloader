...
data class MediaItem(
    ...
    // stream, whose length lives in its manifest) or hasn't run yet. Surfaced in the list so
    ...
    // True from the moment an item capable of having a duration (a real .gif or any video -
    // including one with a <video poster>, which is just a display image and carries no
    // length of its own; only a stream is exempt, see MainActivity.fetchThumbnails) is added
